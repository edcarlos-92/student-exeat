...
if (isset($_GET['search'])) {
	...
	$total_pages_sql = 'SELECT COUNT(*) FROM form t  WHERE Lower(t.form_name) LIKE "%' . $searchKey . '%"';
} else {
	$total_pages_sql = "SELECT COUNT(*) FROM form";
}
...
if (isset($_POST['btnDeleteSelected']) && isset($_POST['checkbox'])) {
	...
	$query = "delete from form where form_id IN ($ids)";
	if ($db->query($query) === TRUE) {
		$log->info('Delete Selected form ' . $ids . ' by ' . $_SESSION["user"]);
		echo "<div class='card mb-3'><div class='alert alert-success'>Selected records deleted successfully</div></div>";
	} else {
		$log->error('Error in deleting selected form ' . $ids . ' by ' . $_SESSION["user"]);
		$log->error($db->error);
		...
		switch ($_GET['action']) {
			case 'delete':
				if ($per == "sa") {
					...
					if ($db->query($query) === TRUE) {
						$log->info('Record ID ' . $id . ' deleted on Form by ' . $_SESSION["user"]);
						echo "<div class='card mb-3'><div class='alert alert-success'>Record deleted successfully</div></div>";
					} else {
						$log->error('Error in deleting record Form by ' . $_SESSION["user"]);
						$log->error($db->error);
						echo "<div class='card mb-3'><div class='alert alert-error'>Error: <br>" . $db->error . "</div></div>";
					}
				} else {
            ...
            <table class="table table-bordered" id="dataTable" width="100%" cellspacing="0">
                ...
                <tbody>
					...
					if (isset($_GET['search'])) {
						...
						$query = 'SELECT  t.* FROM form t  LIMIT ' . $offset . ' , ' . $no_of_records_per_page . '';
					}
					...
					if ($result && $result->num_rows > 0) {
						...
						while ($row = $result->fetch_assoc()) {
							...
							echo '<td style="width:20px"><input class="checkRow" name="checkbox[]" type="checkbox" value="' . $row['form_id'] . '" /></td>';
							echo '<td>' . $row['form_name'] . '</td>';
							if (isset($_GET['search'])) {
								$searchKey = $_GET['search'];
								echo '<td>';
								if ($per == 'sa' || $per == 'a') {
									echo ' <a  type="button" class="btn btn-xs btn-warning" href="update.php?id=' . $row['form_id'] . '"> EDIT </a> </td><td>';
								}
								if ($per == 'sa') {
									echo ' <a  type="button" class="btn btn-xs btn-danger" href="index.php?search=' . $searchKey . '&action=delete&id=' . $row['form_id'] . '" onclick=\'return confirm("DELETING RECORD! Are you sure?")\'>DELETE </a>';
								}
								echo '</td></tr>';
							} else {
								echo '<td>';
								...
								if ($per == 'sa') {
									echo ' <a  type="button" class="btn btn-xs btn-danger" href="index.php?action=delete&id=' . $row['form_id'] . '" onclick=\'return confirm("DELETING RECORD! Are you sure?")\'>DELETE </a>';
								}
								echo '</td></tr>';
							}
						}
